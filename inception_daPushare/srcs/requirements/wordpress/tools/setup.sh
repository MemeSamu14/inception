...
if [ ! -f "$WP_PATH/wp-config.php" ]; then
    ...
    wp core install --url=${DOMAIN_NAME} --title="sfiorini's site" --admin_user=${WORDPRESS_ADMIN_USER} --admin_password=${WORDPRESS_ADMIN_PASSWORD} --admin_email=${WORDPRESS_ADMIN_EMAIL} --allow-root
else
...
wp option update siteurl 'https://sfiorini.42.fr' --allow-root
wp option update home 'https://sfiorini.42.fr' --allow-root
